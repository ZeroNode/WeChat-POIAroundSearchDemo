...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 清空contentview
    for (UIView *any in cell.contentView.subviews){
    ...
    if (tableView == _leftTableView) {
        ...
        //anchor
        if(_currentSelectedMenudIndex ==2 && indexPath.row == 0 ){
            cell.textLabel.text = @"" ;
            
            /* 一级菜单添加搜索 */// UISearchBar的常用方法 搜索框
            UISearchBar *oneSearchBar = [[UISearchBar alloc] init];
            oneSearchBar.frame = CGRectMake(0, 0, self.frame.size.width, 42); // 设置位置和大小
            oneSearchBar.keyboardType = UIKeyboardTypeDefault; // 设置弹出键盘的类型
            oneSearchBar.barStyle = UIBarStyleBlackTranslucent; // 设置UISearchBar的样式
//            oneSearchBar.barStyle = UIBarStyleDefault ;
//            oneSearchBar.backgroundColor = [UIColor clearColor];

            oneSearchBar.placeholder = @"请输入："; // 设置提示文字
            UIView *segment = [[oneSearchBar.subviews objectAtIndex:0].subviews objectAtIndex:0];
            [segment removeFromSuperview];
            oneSearchBar.backgroundColor = [UIColor clearColor];
            
//            CALayer *layer=[oneSearchBar layer];
//            //是否设置边框以及是否可见
...
//             oneSearchBar.showsBookmarkButton = YES; // 设置显示书签按钮
            ...
            for(id cc in [oneSearchBar subviews])
            {
            ...
            [cell.contentView addSubview:oneSearchBar];
        }
    } else {
...
//点击searchbar上的 取消  时调用
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar{
    //开始执行搜索
    [searchBar resignFirstResponder];
//    NSLog(@"dosearch...........%@.",searchBar.text);
    [_delegate menu:self doSearch:searchBar.text];
}
